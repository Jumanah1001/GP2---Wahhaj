"""
5_AHP_Management.py  —  Review and confirm AHP weights
"""
...
<h2 style='font-family:Capriola,sans-serif;color:#1a1a1a;margin-bottom:4px;'>AHP Management</h2>
...
# AHP weights from AHPModel.computeSuitabilityScore
DEFAULT_WEIGHTS = {
...
st.caption("These weights are used by the AHP model to compute the final suitability score. "
...
with col_chart:
    try:
        ...
        labels  = [lbl.split("(")[0].strip() for lbl in DEFAULT_WEIGHTS.keys()]
...
with c2:
    ...
    if not extractor_ready:
        ...
        if st.button("Continue to Run Analysis →", use_container_width=True, type="primary"):
